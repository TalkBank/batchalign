# Malayalam processing reference

The transcription language code is `mal`. For morphology, use the appropriate
CHAT `@Languages:` header; there is no morphotag `--lang` option.

The CLI includes a `malayalam` ASR engine backed by Wav2Vec2 and a Malayalam segmentation implementation. That does not imply a usable Stanza morphology model. Verify the backend required by the task.
...
The [earlier Malayalam study](https://github.com/TalkBank/batchalign/blob/fbaee727d9649f8d4bf1592433fb71f7acce7b12/book/src/batchalign/reference/languages/malayalam.md)
