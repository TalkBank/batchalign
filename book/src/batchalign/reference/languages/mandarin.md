# Mandarin processing reference

The transcription language code is `cmn`. For morphology, use the appropriate
CHAT `@Languages:` header; there is no morphotag `--lang` option.

Use a Mandarin recording language code for transcription. Stanza mapping goes through `ud/lang.py`; the backend normalizes resolved `zh` to `zh-hans`. Do not assume every ISO alias has an alpha-2 mapping.
...
The [earlier Mandarin study](https://github.com/TalkBank/batchalign/blob/fbaee727d9649f8d4bf1592433fb71f7acce7b12/book/src/batchalign/reference/languages/mandarin.md)
